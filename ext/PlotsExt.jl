...
@recipe function plot!(boxset::BoxSet{Box{1,T}}) where {T}
...
#=
@recipe function plot!(coords::AbstractVector{<:Complex})
    seriestype --> scatter
    float.(real.(coords)), float.(imag.(coords))
end

@recipe function plot!(coords::AbstractVector{<:Complex}, heights::AbstractVector{<:Real})
    seriestype --> scatter
