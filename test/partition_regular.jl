...
@testset "internal functionality" begin
    ...
    @testset "point to key" begin
        ...
        @test checkbounds(Bool, partition, GAIO.point_to_key(partition, on_boundary_left))
        @test !isnothing(GAIO.point_to_key(partition, on_boundary_left .+ eps()))
        @test checkbounds(Bool, partition, GAIO.point_to_key(partition, on_boundary_left .+ eps()))
        @test isnothing(GAIO.point_to_key(partition, on_boundary_right))
        @test !checkbounds(Bool, partition, GAIO.point_to_key(partition, on_boundary_right))
        @test !isnothing(GAIO.point_to_key(partition, on_boundary_right .- eps()))
        @test checkbounds(Bool, partition, GAIO.point_to_key(partition, on_boundary_right .- eps()))
        @test checkbounds(Bool, partition, GAIO.point_to_key(partition, on_boundary_right .- 5*eps()))
        @test checkbounds(Bool, partition, GAIO.point_to_key(partition, on_boundary_right .- 10*eps()))
