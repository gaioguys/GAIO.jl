...
function morse_tiles(
        F♯::TransferOperator{B,T,S}, morse_component_map::AbstractVector; 
        dicttype = OrderedDict{keytype(Q),Int}
    ) where {B,T,Q,R,S<:BoxSet{B,Q,R}}

    @assert F♯.domain == F♯.codomain

    P = F♯.domain.partition
    tiles = BoxFun(P, dicttype())

    for (key, comp) in zip(F♯.domain.set, morse_component_map)
        if !iszero(comp)
            tiles[key] = comp
        end
    end
...
function morse_tiles(
        F♯::TransferOperator{B,T,S}, strong_components::Strong_components_output
    ) where {B,T,Q,R,S<:BoxSet{B,Q,R}}
...
function morse_graph_and_tiles(F♯::TransferOperator)
    ...
    morse_tiles_F♯ = morse_tiles(F♯, morse_component_map_F♯)
